...
from .session_end_pending import (
...
def _session_end_lock_attribution_detail(error: BaseException | None) -> str:
    """Extract only allowlisted coordinator metadata from an exception."""
    if error is None:
        return "-"
    text = " ".join(str(error).split())
    fields: list[str] = []
    patterns = (
        ("owner_thread_id", r"\bowner_thread_id=([0-9]+)\b"),
        ("owner_thread_name", r"\bowner_thread_name=([A-Za-z0-9_.:-]{1,96})\b"),
        ("owner_operation", r"\bowner_operation=([A-Za-z0-9_.:-]{1,96})\b"),
        ("owner_age_s", r"\bowner_age_s=([0-9]+(?:\.[0-9]+)?)\b"),
    )
...
def _log_session_end_deferred_event(
    ...
    *,
    ...
    logger.log(
        ...
        error_type,
        _session_end_lock_attribution_detail(error),
    )
...
class LCMEngine(CompactionMixin, ResetStateMixin, ReconcileMixin, AuxiliarySessionMixin, PlaceholderLedgerMixin, BypassMixin, ContextEngine):
    ...
    def _session_end_tool_replay_plan(
        self,
        session_id: str,
        messages: List[Dict[str, Any]],
        *,
            ...
            if role == "tool":
                ...
                if call_ids and call_ids.issubset(stored_assistant_ids):
                    ...
                    if isinstance(tool_calls, list):
                        ...
                        replacement["tool_calls"] = kept_calls
                        rewritten[index] = replacement
        return replayed, rewritten
    ...
    def _session_end_replayed_tool_indexes(
        ...
        *,
        ...
        replayed, _ = self._session_end_tool_replay_plan(
            session_id,
            messages,
            conversation_id=conversation_id,
        )
    ...
    def _append_off_current_session_end_suffix(
        ...
        *,
        ...
        if not session_id or not suffix:
            return []
        replayed_tool_indexes, rewritten_tool_messages = self._session_end_tool_replay_plan(
            session_id,
            suffix,
            conversation_id=conversation_id,
        )
        ...
        for index, msg in enumerate(suffix):
            if index in replayed_tool_indexes:
                continue
            msg = rewritten_tool_messages.get(index, msg)
    ...
    def on_session_end(self, session_id: str, messages: List[Dict[str, Any]]) -> None:
        ...
        if (
            ...
            return
        session_end_flush_lock = self._storage_owner._session_end_flush_lock
    ...
    def shutdown(self):
        ...
        try:
            self._unregister_active_engine_binding()
            drain_done = getattr(self, "_session_end_drain_done", None)
